...
void add_snippet(snippet_entry_t snippet) {
    if (snippet_collection.snippet_count < MAX_SNIPPETS) {
        ...
        // Copy the snippet directly to our flat array
        strncpy(snippet_collection.snippet_arr[i].trigger, snippet.trigger, SNIP_BUFFER_SIZE - 1);
        snippet_collection.snippet_arr[i].trigger[SNIP_BUFFER_SIZE - 1] = '\0';  // Ensure null termination

        strncpy(snippet_collection.snippet_arr[i].snippet, snippet.snippet, MAX_SNIPPET_LEN - 1);
        snippet_collection.snippet_arr[i].snippet[MAX_SNIPPET_LEN - 1] = '\0';  // Ensure null termination

        // Store the end code
        snippet_collection.snippet_arr[i].end_code = snippet.end_code;
