...
__attribute__((weak)) bool achordion_chord(uint16_t tap_hold_keycode,
  ...
  if (tap_hold_keycode == OSL(1) ||
      ...
      tap_hold_keycode == LT(3, KC_N) ||
      tap_hold_keycode == MT(MOD_LSFT, QK_AREP) ||
      tap_hold_keycode == LT(1, QK_REP) ||
      tap_hold_keycode == LT(5,KC_SPACE) ||
      tap_hold_keycode == MT(MOD_LSFT, KC_ENTER) ||
      tap_hold_keycode == MT(MOD_MEH, QK_REP)
      ){
    return true;
  }
